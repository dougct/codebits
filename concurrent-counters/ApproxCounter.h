...
class ApproxCounter {
 ...
 public:
  ...
  int64_t update(int64_t amount) {
    ...
    const uint64_t curr_update = num_updates_.fetch_add(1, std::memory_order_seq_cst);
    const uint64_t idx = curr_update % local_counters_.size();
    {
      ...
      if (num_updates_ >= threshold_) {
        global_counter_.fetch_add(local_counters_[idx], std::memory_order_relaxed);
  ...
  int64_t collect() {
    ...
    global_counter_.fetch_add(sum, std::memory_order_seq_cst);
